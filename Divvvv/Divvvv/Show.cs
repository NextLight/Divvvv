...
namespace Divvvv
{
    class Show
    {
        ...
        {
            var s = await HttpDownloader.GetStringAsync($"https://www.vvvvid.it/vvvvid/ondemand/{Id}/seasons/?conn_id={connId}");
            Series = await Task.WhenAll(
                s.ReMatchesGroups("\"season_id\":(.+?),.*?\"name\":\"(.+?)\"")
        ...
        {
            var j = new Json(await HttpDownloader.GetStringAsync($"https://www.vvvvid.it/vvvvid/ondemand/{Id}/season/{serieId}?conn_id={connId}"));
