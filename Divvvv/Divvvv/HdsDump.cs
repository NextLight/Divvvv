...
namespace HDS
{
    ...
    internal class HdsDump
    {
        ...
        public static void Message(string msg)
        {
            ...
            var colorsStack = new List<ConsoleColor>();
    ...
    internal class HTTP
    {
        ...
        private readonly bool notUseProxy;
        ...
        public string ResponseText => Encoding.ASCII.GetString(ResponseData);
        ...
        private HttpWebRequest CreateRequest()
        {
            ...
            if ((Proxy != "") && !notUseProxy)
            {
                ...
                var myProxy = new WebProxy {Address = new Uri(Proxy)};
                if (ProxyUsername != "")
    ...
    internal class F4F
    {
        ...
        private static long ReadInt(int size, ref byte[] bytesData, long pos)
        {
            ...
            int n = size / 8;
            for (byte i = 0; i < n; i++)
                res |= ((long) bytesData[pos + i]) << (n - 1 - i) * 8;
            return res;
        }
        ...
        private int FindFragmentInTabe(int needle) => fragTable.FindIndex(m => m.FirstFragment == needle);
        ...
        private void ParseManifest(string manifestUrl)
        {
            ...
            var keys = new string[media.Keys.Count];
        ...
        private void ThreadDownload()
        {
            while (fragmentsComplete < fragCount)
            {
                ...
                {
                    var t = new Thread(StartNewThread2DownloadFragment) {IsBackground = true};
                    t.Start();
                }
                Thread.Sleep(300);
            }
        ...
        public IEnumerable<DownloadedFragmentArgs> DownloadFragments()
        {
            ...
            if (usedThreads)
            {
                ...
                var mainThread = new Thread(ThreadDownload) {IsBackground = true};
                mainThread.Start();
            }
            ...
            while (fragNum < fragCount && !Exit)
            {
                ...
                string remaining = $"<c:DarkCyan>Time remaining: </c>{timeRemaining.Hours:00}<c:Cyan>:</c>{timeRemaining.Minutes:00}<c:Cyan>:</c>{timeRemaining.Seconds:00}";
                ...
                else
                {
                    ...
                    var retCode = cc.Get(GetFragmentUrl(segNum, fragNum));
                    if (retCode != 200)
                    {
        ...
        private void WriteMetadata()
        {
            ...
            var metadata = new byte[tagHeaderLen + mediaMetadataSize + 4];
        ...
        private void DecodeFragment(ref byte[] frag, bool notWrite = false)
        {
            ...
            while (fragPos < fragLen)
            {
                ...
                currentTS = packetTS;
                var lastTS = prevVideoTS >= prevAudioTS ? prevVideoTS : prevAudioTS;
                var fixedTS = lastTS + Constants.FRAMEFIX_STEP;
                ...
                switch (packetType)
                {
                    ...
                    case Constants.VIDEO:
                        if (packetTS > prevVideoTS - fixWindow)
                        {
                            ...
                            if (packetSize > 0)
                            {
                                ...
                                if (CodecID != Constants.CODEC_ID_AVC || AVC_PacketType != Constants.AVC_SEQUENCE_HEADER && AVC_PacketType != Constants.AVC_SEQUENCE_END && !prevAVC_Header)
                                {
                    ...
                    default:
                        switch (packetType)
                        {
                            case 10:
                            case 11:
                                HdsDump.Quit("<c:Red>This stream is encrypted with <c:Magenta>Akamai DRM</c>. Decryption of such streams isn't currently possible with this program. Not yet.");
                                break;
                            case 40:
                            case 41:
                                HdsDump.Quit("<c:Red>This stream is encrypted with <c:Magenta>FlashAccess DRM</c>. Decryption of such streams isn't currently possible with this program. Not yet.");
                                break;
                            default:
                                HdsDump.Quit("<c:Red>Unknown packet type <c:Magenta>" + packetType + "</c> encountered! Encrypted fragments can't be recovered. I'm so sorry.");
                                break;
                        }
                        break;
                }
